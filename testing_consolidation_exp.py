...
def routing(uploaded_file):
    def plot_vehicle_route_with_navigation(df, vehicle_name):
        ...
        for index, row in vehicle_df.iterrows():
            ...
            <b>TFR value (Rs):</b> {row['TFR value (Rs)']}<br>
    ...
    def create_data_model(distances, coordinates):
        ...
        data["demands"] = list(map(int, places.tfr.to_list())) 
        data["vehicle_capacities"] = [200000] * Ravi_Count + [500000] * Shehzore_Count + [1000000] * Mazda_Count + [120000] * EV_Count
    ...
    def main():
        ...
        if solution:
            ...
            dff1 = dff1.rename(columns={'Load': 'TFR value (Rs)'})
            ...
            # Rename 'Load' column to 'TFR value (Rs)' and calculate TFR value per order for each vehicle
            new_df = new_df.rename(columns={'Load': 'TFR value (Rs)'})
            new_df['TFR value (Rs)'] = new_df.groupby('Vehicle')['TFR value (Rs)'].diff().fillna(new_df['TFR value (Rs)'])
            ...
            new_df = new_df[['Order ID', 'Order Status', 'Order Delivered At', 'Vehicle', 'Priority', 'TFR value (Rs)', 'Distance (km)', 'lat', 'lng']]
    ...
    places = pd.read_csv(uploaded_file)

    # Create a new row as a pandas Series
    new_row = pd.Series({'Order ID': 0, 'Order Delivered At': 0, 'Order Status': 0, 'lat': 24.8, 'lng': 67.1, 'tfr': 0})
    ...
    #Ravi/Pickup
    Ravi_Rent = 2050 # (Rupees)
    Ravi_Mileage = 9 # (KM/L)
    Ravi_Count = st.number_input("Input number of RV available: ", min_value=0, max_value=100, value=50)#100 #int(input("Input number of Ravi available: "))
    
    #Shehzore
    Shehzore_Rent = 3500 # (Rupees)
    Shehzore_Mileage = 6 # (KM/L)
    Shehzore_Count = st.number_input("Input number of Shehzore available: ", min_value=0, max_value=100, value=50)#100 #int(input("Input number of Shehzore available: "))

    #Mazda
    Mazda_Rent = 5000 # (Rupees)
    Mazda_Mileage = 5 # (KM/L)
    Mazda_Count = st.number_input("Input number of Mazda available: ", min_value=0, max_value=100, value=50)#100 #int(input("Input number of Mazda available: "))

    #EV
    EV_Rent = 1500 # (Rupees)
    EV_Mileage = 70 # (KM/L)
    EV_Count = st.number_input("Input number of EV available: ", min_value=0, max_value=100, value=50)#100 #int(input("Input number of EV available: "))
